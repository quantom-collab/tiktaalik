module integration
  ...
  contains
    ...
    function integrate2(func, x, xi) result(integral)
        real(dp), external :: func
        real(dp), intent(in) :: x, xi
        real(dp) :: integral
        !
...
recursive subroutine qags(f,a,b,epsabs,epsrel,result,abserr,   &
! optional
                          oneval,oifail)
!-------------------------------------------------------------------------------
...
! and test for accuracy.
     ...
     if ( abs ( rlist(maxerr) - area12) > 1.0e-05 * abs(area12) &
          .or. erro12 < 9.9e-01_dp * errmax ) goto 10
